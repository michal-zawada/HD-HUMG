...
params.vcfTodb = "***ref_db/vcf_upload/vcf_to_db.py"
...
//params.reads = "***fastq_test_data/trio_WES_fastq/AS*/fastq/*_R{1,2}.fastq.gz"  // 3 pairs of fastq from one family - trio
params.reads = "***fastq_test_data/trio_WES_test_fastq/*_R{1,2}.fastq" // 3 pairs of TEST fastq from one family - trio
//params.samplesConfig="***current_run/samplesToRun_*.csv"

params.samplesConfig="***current_run_test/samplesToRun_*.csv"
...
params.hapmap = "***/hg38/GATK_resource_bundle/hapmap_3.3.hg38_noChr.vcf.gz"
params.omni = "***/hg38/GATK_resource_bundle/1000G_omni2.5.hg38_noChr.vcf.gz"
params.g1000G = "***/hg38/GATK_resource_bundle/1000G_phase1.snps.high_confidence.hg38_noChr.vcf.gz"
params.mills = "***/hg38/GATK_resource_bundle/Mills_and_1000G_gold_standard.indels.hg38_noChr.vcf.gz"
...
Channel
    .fromPath(params.samplesConfig)
    .splitCsv(header:true)
    .map{ row-> tuple(row.order_id, row.index_patient, row.sort_value) }
    .set { merge_family_gvcfs }

Channel
    .fromPath(params.samplesConfig)
    .splitCsv(header:true)
    .map{ row-> tuple(row.order_id, row.read1) }
    .set { gvcfs }


//merge_family_gvcfs.join(gvcfs).set{to_collect}
//to_collect.println()

//collected=to_collect.groupTuple(by:1)
//collected.println()


//to_collect.collectFile(storeDir: "***results/nextflow_results/trio_test", newLine:true, sort:{'index'}) { order_id, index_patient, sort_value, read1  -> ["${index_patient}.list", "${read1}"]}.set{collected}
//collected.println()
...
process haplotype_calling {
  ...
  output:
  tuple val(sample_id), file('*.g.vcf.gz'), file('*.g.vcf.gz.tbi') into gvcf_raw
  //tuple val(sample_id) into gvcf_tbi_raw
...
/*
In order to perform trio analysis, we need to combine gvcf files and then run genotype on combined gvcf (gatk: combineGVCF, genotypeGVCF)
The number of patients for combineGVCF process might be different (trio, quadro, duo etc.) we cannot use simply file parameter as input, but we need to use list of files, which might be longer or shorter.
The list of files needs to be saved as file with end ".list" and then used as input in combineGVCF. It means, that for each family we will have one "filename.list" file.
Because before the combineGVCF the processes for each patient (even within one family) are runing in pararel, the data for each patient is ready at different time.
We need to collect the gvcfs from haplotype calling, which belong to one family. We can do it using collectFile. 
collectFile will change the output chanel from haplotype caller where every patient is separate and group them into families.
collectFile needs to know according to what the files should be grouped. For us, the should be grouped according to family_id. 
In collectFile the files are grouped according to a value in chanel, which is used as newly created file name (${index_patient}.list.unsorted). 
We want to group according to family_id, but we want to have a file name "filename.list" for combineGVCF with index patient as name, not family_id.
To achive this, we need to add to chanel gvcf_raw from haplotype_calling (for each gvcf) a value  with index patient file name. We add it in merge_family_gvcf.join ... 
Then for index patient in the chanel the begining of the gvcf and index_patient will be the same, but for example for mother it will be different: gvcf form mother and index_patient form index.
Note that in collectFile we are not using file(gvcf), but only gvcf, because we don't want to collect the inputs of the files to one file, but the file name only. 

Another value we add in merge_family_gvcf.join ... is sort_value. The purpose is as follow:
The gvcf files in chanel from haplotype_calling are not always ordered as we want (index, mother, father), so they are not then ordered in the filename.list. 
That's why with collectFile the output file is called "unsorted" - ${index_patient}.list.unsorted. In combined gvcf file the columns of genotype are in the same order like entered in the given filename.list,
so we want that index patient is in the first column, then mother, father, sibling etc. 
After creating filename.list.unsorted file, we have in the file a sort_value, according to which the files should be sorted. 
The sort_value should be given not in nextflow, but any other way before starting nextflow using python and saved in config csv with file names for the run. -> to do
Then the filenames in filename.list.unsorted are sorted sort_gvcfs_lists process only file names and saved in filename.list

!!!!!!!! TO DO !!!!!!!!!!
in seq_run_upload_auto.py script program the sort_value how it's added to the files and saved in the csv file.list

*/

merge_family_gvcfs.join(gvcf_raw).into{gvcf_for_list;gvcf_for_join}
//gvcf_for_list.println()
//[AS-339716-LR-42192, AS-339715-LR-42192, ***results/nextflow_results/trio_test/work/8a/e3936ea9a0c177d0c03c3286de57ca/AS-339716-LR-42192.g.vcf.gz, ***results/nextflow_results/trio_test/work/8a/e3936ea9a0c177d0c03c3286de57ca/AS-339716-LR-42192.g.vcf.gz.tbi]

gvcf_for_list.collectFile(storeDir: "***results/nextflow_results/trio_test", newLine:true, sort:true) { sample_id, index_patient, sort_value, gvcf, gvcf_tbi -> ["${index_patient}.list.unsorted", "${gvcf}\t${sort_value}"]}.set{gvcfs_list}
gvcfs_list.map{it -> [it.getSimpleName(),it]}.set{gvcfs_list_with_index}


process sort_gvcfs_lists {
  tag "$sort_gvcfs_lists"

  input:  
  tuple val(sample_id), file(gvcfs_list) from gvcfs_list_with_index

  output:
  tuple val(sample_id), file('*.list') into gvcfs_list_sorted
  """  
  sort -k 2 ${gvcfs_list} | cut -f1 > ${sample_id}.list
  """
}


gvcf_for_join=gvcf_for_join.groupTuple(by:1)
//grouped by second value - index_patient
//gvcf_for_join.println()
//[[AS-339716-LR-42192, AS-339715-LR-42192], AS-339715-LR-42192, [2, 1], [***results/nextflow_results/trio_test/work/8a/e3936ea9a0c177d0c03c3286de57ca/AS-339716-LR-42192.g.vcf.gz, ***results/nextflow_results/trio_test/work/e5/107ee4403acc169c12cd0da2984828/AS-339715-LR-42192.g.vcf.gz], [***results/nextflow_results/trio_test/work/8a/e3936ea9a0c177d0c03c3286de57ca/AS-339716-LR-42192.g.vcf.gz.tbi, ***results/nextflow_results/trio_test/work/e5/107ee4403acc169c12cd0da2984828/AS-339715-LR-42192.g.vcf.gz.tbi]]


tbi_for_join=gvcf_for_join.map{sample_id, index_patient, sort_value, gvcf, tbi -> ["${index_patient}", "${tbi}"]}
//selecting tbi files to join with file.list  
//tbi_for_join.println()
//[AS-339715-LR-42192, [***results/nextflow_results/trio_test/work/8a/e3936ea9a0c177d0c03c3286de57ca/AS-339716-LR-42192.g.vcf.gz.tbi, ***results/nextflow_results/trio_test/work/e5/107ee4403acc169c12cd0da2984828/AS-339715-LR-42192.g.vcf.gz.tbi]]

gvcfs_list_sorted.join(tbi_for_join).set{gvcf_list_tbi}
//gvcf_list_tbi.println()
...
process combine_gvcfs {
  ...
  input:  
  tuple val(sample_index), file(gvcfs_list), file(tbi) from gvcf_list_tbi

  output:
  tuple val(sample_index), file('*.g.vcf.gz') into gvcf_combined

  """
  $params.gatk4 CombineGVCFs \
  -R $params.hg38 \
  -V $gvcfs_list \
  -O ${sample_index}.g.vcf.gz \
  """
}

//gvcf_combined.println()

//now GenotypeGVCF

//Further processes are not developed yet.
...
samples in parallel might cause some errors. You can add "maxForks" in the process which shouldn't be parallelized
  ...
  dbnsfp -v -db ***/dbnsfp/dbNSFP4.0a.gz \
